# `automation init` Command Guide

Complete guide to using the `automation init` command for automated repository setup.
...
automation init --non-interactive
```
...
automation init --backend encrypted
```
...
2. Run the automation daemon:
   automation --config automation/config/automation_config.yaml daemon --interval 60
...
automation init --non-interactive --backend environment
```

### Example 3: Custom Config Location

```bash
automation init --config-path config/production.yaml
```
...
automation init --no-setup-secrets
```
...
- Used when you run `automation` commands locally
  ...
  process:
    ...
    steps:
      ...
      - name: Process Issues
        ...
        run: |
          automation --config automation/config/automation_config.yaml process-all
```
...
```bash
cd ~/projects/repo1
automation init --config-path config/repo1.yaml

cd ~/projects/repo2
automation init --config-path config/repo2.yaml
```
...
cd ~/org/repo2
automation init  # Uses same keyring credentials
...
automation init --help

# View credentials help
automation credentials --help
