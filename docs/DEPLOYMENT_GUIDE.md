...
Check that the sapiens workflow wrapper is present:

```bash
ls -la .gitea/workflows/sapiens.yaml
# The thin wrapper workflow that calls the reusable sapiens-dispatcher.
# This is generated by `sapiens init` or `sapiens update --workflows`.
#
# The wrapper references:
# savorywatt/repo-sapiens/.github/workflows/sapiens-dispatcher.yaml@v0.5.1
```

The new workflow architecture uses a single thin wrapper (~20 lines) that calls
the reusable dispatcher workflow from the repo-sapiens repository. This means:
- No more copying multiple workflow files
- Automatic updates by bumping the version tag
...
- [ ] Workflow file present: `.gitea/workflows/sapiens.yaml`
