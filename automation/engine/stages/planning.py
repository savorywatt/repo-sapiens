...
from automation.models.domain import Issue, Plan
...
- To approve: Close this issue and add label `{self.settings.tags.ready_to_implement}` to #{issue.number}
- To request changes: Comment with feedback and add label `{self.settings.tags.needs_attention}` to #{issue.number}
