...
class QAStage(WorkflowStage):
    ...
    async def execute(self, issue: Issue) -> None:
        ...
        try:
            ...
            subprocess.run(["git", "checkout", "-B", pr.head, f"origin/{pr.head}"],
            ...
            if test_result.get("command") == "none":
                ...
                await self.git.add_comment(
                    issue.number,
                    f"📝 **No Tests Found - Creating Unit Tests**\n\n"
                    f"I'll analyze the code and create unit tests.\n\n"
                    f"🤖 Posted by Builder Automation"
                )
                ...
                if test_creation_result["success"]:
                    ...
                    if status_result.stdout.strip():
                        commit_message = f"test: Add unit tests for PR #{pr.number}\n\n🤖 Auto-generated by Builder QA"
                        subprocess.run(
            ...
            # Post results
            result_lines = [
                f"{'✅' if qa_passed else '❌'} **QA {'Passed' if qa_passed else 'Failed'}**\n",
                f"Branch: `{pr.head}`\n",
                f"PR: #{pr.number}\n",
                "",
                "## Build",
                f"Status: {'✅ Success' if build_result['success'] else '❌ Failed'}",
            ]
            ...
            result_lines.extend([
                "",
                "## Tests",
                f"Status: {'✅ Success' if test_result['success'] else '❌ Failed'}",
            ])
            ...
            # Update labels
            updated_labels = [l for l in issue.labels if l != "requires-qa"]
...
"""
    ...
    async def _run_test(self, project_dir: Path) -> dict:
        ...
        for cmd, indicator_file in test_commands:
            ...
            if indicator_path.exists() or (indicator_path.is_dir() if indicator_path.name == "tests" else False):
